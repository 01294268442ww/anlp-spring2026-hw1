...
def apply_rotary_emb(
    ...
    freq_seq = torch.arange(0, head_dim, 2, device=device).float()
    ...
    cos = reshape_for_broadcast(cos, query_real).float()
    sin = reshape_for_broadcast(sin, query_real).float()
    ...
    query_out = torch.stack(
        [query_out_real, query_out_imag],
        dim=-1
    ).flatten(-2)

    key_out = torch.stack(
        [key_out_real, key_out_imag],
        dim=-1
    ).flatten(-2)
